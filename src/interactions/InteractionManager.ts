	...
	implements LifeTimeObject
{
	...
	private InstancedMeshManagers: {
		name: string;
		manager: InstancedMeshManager;
	}[] = [];
	...
	declare private dialogsAudio: {
		[key: string]: {
			[value: string]: { audio: string; dialog: string; speaker: string };
		};
	};
	...
	constructor() {
		...
		this.experience.canvas.addEventListener(
			"mousemove",
			this.updateMouseScreenPosition
		);
		this.experience.canvas.addEventListener(
			"mouseup",
	...
	updateInteractableObjects(
		newResources: { name: string; resourceName: string }[]
	) {
		...
		const buttonContainer = document.getElementById(this.buttonContainerId);
		if (!buttonContainer)
			throw new Error("Can't refresh buttons: buttonContainer is null");
		...
		newResources.forEach((pair) => {
			...
			this.InstancedMeshManagers.push({
				name: pair.name,
				manager: instancedMeshManager,
			});
