...
export default class InteractionManager extends EventEmitter implements LifeTimeObject {
	...
	declare interactableObjects: { name: string; resourceName: string }[];
	...
	// Subtitle manager
	declare private subtitle: SubtitleManager;
	declare private dialogsAudio: {
	...
	constructor() {
		...
		// A remettre pour des tests rapides
		// this.interactableObjects = [
		//     { name: "dinosaure", resourceName: "mushroomPaintedModel" },
		//     { name: "mushroom2", resourceName: "mushroomModel" },
		//     { name: "mushroomCouc", resourceName: "mushroomPaintedModel" },
		// ]

		// objet vide qui affiche les objets à placer pour une scene / step donnée
		this.interactableObjects = [];
		...
		this.setDebugObject();
		this.updateInteractableObjects(this.interactableObjects);
		// this.selectedObject = "mushroom2";
	}

	// Méthode utilisé pour ajouter / enlever les objets en fonction de SceneManager
	addInteractableObject(name: string, resourceName: string) {
		this.interactableObjects.push({ name, resourceName });
		this.updateInteractableObjects(this.interactableObjects);
	}

	deleteInteractableObject(name: string) {
		this.interactableObjects = this.interactableObjects.filter((obj) => obj.name !== name);
		this.updateInteractableObjects(this.interactableObjects);
	}
	...
	updateInteractableObjects(newResources: { name: string; resourceName: string }[]) {
		...
		newResources.forEach((pair) => {
			...
			const instancedMeshManager = new InstancedMeshManager(
				resource.scene.children[0] as THREE.Mesh
			);
	...
	addSelectedObject = (e: MouseEvent) => {
		...
		const instancedMeshManager = this.instancedMeshManagers.find(
			(m) => m.name === this.selectedObject
		)?.manager;
		...
		this.trigger("onObjectPlaced", [this.selectedObject]);
	};
