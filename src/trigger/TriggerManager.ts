...
export default class TriggerManager extends EventEmitter {
    ...
    init() {
        ...
        this.createTriggerZone("chamberDoor", { x: 6, y: 1.9, z: 0 }, { width: 1, height: 3, depth: 2 }, () =>
            this.sceneManager.addActiveStep(10)
        );
        // Entrée de la première clairière
        this.createTriggerZone("firstClearingEntry", { x: 9, y: 2, z: -61 }, { width: 1, height: 5, depth: 2 }, () =>
            this.sceneManager.addActiveStep(11)
        );
        // Exit clairière
        this.createTriggerZone("firstClearingExit", { x: 44, y: 2, z: -59 }, { width: 1, height: 5, depth: 2 }, () =>
            this.sceneManager.addActiveStep(16)
        );
        // orage chemin droite
        this.createTriggerZone("storm", { x: 79, y: 9, z: -44 }, { width: 1, height: 5, depth: 2 }, () =>
            this.sceneManager.addActiveStep(99)
        );

        // orage chemin gauche
        this.createTriggerZone("storm", { x: 74, y: 8, z: -79 }, { width: 1, height: 5, depth: 2 }, () =>
            this.sceneManager.addActiveStep(99)
        );

        // Entrée de la deuxième clairière
        this.createTriggerZone("secondClearingEntry", { x: 100, y: 11, z: -12 }, { width: 3, height: 5, depth: 2 }, () =>
            this.sceneManager.addActiveStep(19)
        );

        // Deuxième entrée de la deuxième clairière
        this.createTriggerZone("secondClearingEntry", { x: 114, y: 12, z: -13 }, { width: 3, height: 5, depth: 2 }, () =>
            this.sceneManager.addActiveStep(19)
        );
    ...
    checkTrigger(playerBox: THREE.Box3, triggerBox: TriggerZone) {
        ...
        if (isIntersecting && !triggerBox.isInZone) {
            ...
            if (triggerBox.name === "storm") {
                this.allTriggers.filter((t) => t.name === "storm")
                    .forEach((t) => {
            ...
            if (triggerBox.name === "secondClearingEntry") {
                this.allTriggers.filter((t) => t.name === "secondClearingEntry")
                    .forEach((t) => {
                        this.removeTriggerZone(t.mesh);
                    });
                this.allTriggers = this.allTriggers.filter((t) => t.name !== "secondaryClearingEntry");
            }
