import { Actor, Experience, Floor, World } from "@plugins/baseExperience";
...
export default class BlockingWorld extends World {
	...
	init() {
		...
		const mushroom1 = new InteractableObject(
			"champignon",
			this.resources.items.mushroomPaintedModel as GLTF,
			true
		);
		const mushroom2 = new InteractableObject(
			"champignon",
			this.resources.items.mushroomModel as GLTF,
			true,
			false,
			this.resources.items.mushroomCollider as GLTF
		);
	...
	update() {
		...
		if (this.cloud) this.cloud.update();
